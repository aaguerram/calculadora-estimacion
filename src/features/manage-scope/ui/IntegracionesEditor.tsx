...
export function IntegracionesEditor({
  ...
  return (
    <div className={styles.bloque}>
      <h4 className={styles.titulo}>Integraciones ({alcance.integraciones.length})</h4>

      {alcance.integraciones.length === 0 ? (
        <p className={styles.vacio}>
          El recargo por contraparte externa (×1.4) y por falta de sandbox (×1.3) es lo
          que más se subestima: no es código, es coordinación y bloqueos.
        </p>
      ) : (
                ...
                <tr key={integracion.id}>
                  ...
                  <td>
                    <Checkbox
                      ...
                      onChange={(_e, { checked }) =>
                        void mutar(() =>
                          actualizarIntegracion(integracion.id, { esExterna: checked }),
                        )
                      }
                    />
                  </td>
                  <td>
                    <Checkbox
                      ...
                      onChange={(_e, { checked }) =>
                        void mutar(() =>
                          actualizarIntegracion(integracion.id, { tieneSandbox: checked }),
                        )
                      }
                    />
                  </td>
                  <td className={styles.numero}>
                    <NumberInput
                      ...
                      onBlur={(e) => {
                        ...
                        void mutar(() =>
                          actualizarIntegracion(integracion.id, { usos: valor }),
                        )
                      }}
                    />
                  </td>
                  <td className={styles.acciones}>
                    <Button
                      ...
                      type="button"
                      disabled={guardando}
                      onClick={() => void mutar(() => eliminarIntegracion(integracion.id))}
                    >
                      Eliminar
                    </Button>
                  </td>
      ...
      <div className={styles.altaForm}>
        ...
        <div className={styles.campo}>
          <NumberInput
            ...
            max={99}
            value={usos}
            onChange={(_e, { value }) => setUsos(Math.max(1, Number(value) || 1))}
          />
        </div>
