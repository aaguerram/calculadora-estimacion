...
import type { AlcanceDeProyecto, PosturaDriver } from '@/entities/project-scope'
...
export function DriversEditor({ alcance, guardando, mutar }: DriversEditorProps) {
  ...
  return (
    <div className={styles.bloque}>
      ...
      {CATALOGO_DRIVERS.map((definicion) => {
        const guardado = alcance.drivers.find((d) => d.clave === definicion.clave)
        ...
        return (
          <div key={definicion.clave} className={styles.driver}>
            <div className={styles.driverCabecera}>
              <span className={styles.driverEtiqueta}>{definicion.etiqueta}</span>
      ...
      <p className={styles.factor}>
        Factor de proyecto: <strong>×{factor.toFixed(2)}</strong> — los deltas se{' '}
        <strong>suman</strong>. Multiplicarlos produciría ×
        {alcance.drivers.reduce((t, d) => t * (1 + d.delta), 1).toFixed(2)} con las mismas
        respuestas, que es como se infla una estimación paramétrica.
      </p>
    </div>
  )
}
