import { Button, Checkbox, NumberInput, Select, SelectItem, TextInput } from '@carbon/react'
import { useState } from 'react'

import { ETIQUETA_TIPO, TIPOS_COMPONENTE } from '@/entities/estimation-model'
import type { StackTecnologico, TipoComponente } from '@/entities/estimation-model'
import {
...
export function ComponentesEditor({ alcance, guardando, mutar }: ComponentesEditorProps) {
  ...
  const agregar = async () => {
    ...
    await mutar(() => crearComponente(alcance.proyectoId, { nombre, tipo, stack, esNuevo }))
    setNombre('')
  }

  return (
    <div className={styles.bloque}>
      <h4 className={styles.titulo}>Componentes ({alcance.componentes.length})</h4>

      {alcance.componentes.length === 0 ? (
        <p className={styles.vacio}>
          Sin componentes no hay streams, y sin streams no hay ruta crítica. Empieza aquí.
        </p>
      ) : (
                ...
                <tr key={componente.id}>
                  ...
                  <td>
                    <Checkbox
                      ...
                      onChange={(_e, { checked }) =>
                        void mutar(() =>
                          actualizarComponente(componente.id, { esNuevo: checked }),
                        )
                      }
                    />
                  </td>
                  <td className={styles.numero}>
                    <NumberInput
                      ...
                      onBlur={(e) => {
                        ...
                        void mutar(() =>
                          actualizarComponente(componente.id, { capDevs: valor }),
                        )
                      }}
                    />
                  </td>
                  <td className={styles.acciones}>
                    <Button
                      ...
                      type="button"
                      disabled={guardando}
                      onClick={() => void mutar(() => eliminarComponente(componente.id))}
                    >
                      Eliminar
                    </Button>
                  </td>
